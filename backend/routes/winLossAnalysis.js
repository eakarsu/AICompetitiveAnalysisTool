/**
 * Custom Feature: win-loss-analysis
 ...
 * Integration credentials: process.env.FEATURE_WIN_LOSS_ANALYSIS_KEY
...
const FEATURE_KEY = process.env.FEATURE_WIN_LOSS_ANALYSIS_KEY;
const SYSTEM_PROMPT = `You are an expert assistant specialized in: Win/loss analysis automation.
...
async function callLLM(userPayload) {
  ...
  const response = await fetchFn('https://openrouter.ai/api/v1/chat/completions', {
    ...
    headers: {
      ...
      'X-Title': 'AICompetitiveAnalysisTool - win-loss-analysis',
    },
    body: JSON.stringify({
...
router.post('/win-loss-analysis', auth, async (req, res) => {
  try {
    const payload = req.body || {};
    if (!payload || Object.keys(payload).length === 0) {
      return res.status(400).json({ error: 'Request body is required' });
    }
    if (!FEATURE_KEY) res.set('X-Feature-Credentials-Missing', 'FEATURE_WIN_LOSS_ANALYSIS_KEY');
    const ai = await callLLM({ feature: 'win-loss-analysis', goal: 'Win/loss analysis automation', input: payload });
    try {
      if (pool && pool.query) {
        await pool.query(
          ...
          ['win-loss-analysis', JSON.stringify(payload), JSON.stringify(ai.result), ai.model, req.user?.id || null]
        );
      }
    } catch (persistErr) {
      console.warn('[win-loss-analysis] persistence skipped:', persistErr.message);
    }
    return res.json({
      ok: true,
      feature: 'win-loss-analysis',
      endpoint: '/api/ai/win-loss-analysis',
      ai_result: ai.result,
      model: ai.model,
      tokens: ai.tokens,
    ...
    console.error('[win-loss-analysis] error:', err.message);
    return res.status(err.statusCode || 500).json({ error: err.message || 'Internal error' });
  }
});

router.get('/win-loss-analysis/health', (req, res) => {
  res.json({
    feature: 'win-loss-analysis',
    endpoint: '/api/ai/win-loss-analysis',
